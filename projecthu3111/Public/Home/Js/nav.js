...
$(function () {
	   		...
	   		$('.add-group-sub').click(function(){
		   		$.post(groupadd,{name:gname},function(data){
		   			if(data>0){
		   				...
		   				//在默认分组下头部新增分组
		   				var addggp = $('<option value='+data+'>'+gname+'</option>');
		   				$('.addgrouplist li:eq(0)').append(agroup);
		   				$('.sel-group').find('select').append(addggp);
   ...
   $('#huiyuan #huimain ul .other').click(function(){
   		$('input[name="huiyuanname"]').val('请输入TA的昵称');
   		$('input[name="huiyuanname"]').focus(function(){
   			$('input[name="huiyuanname"]').val('');
   		})
   });
   $('input[name="buytime"]').blur(function(){
	   	var yuan = $('input[name="buytime"]').val();
	   	$('#huiyuan #huimain ul .yuan i').html(parseInt(yuan)*10);
   })
...
//我自己的实时私信息提醒
	...
	setInterval(function(){
		$.post(newletter,{},function(data){
			if(data.newletter>0){
		 		$('.newmessagenum').remove();
				var newmessagenum = $('<i>'+data.newletter+'</i>');
				newmessagenum.addClass('newmessagenum');
		 		$('.icon-msg').append(newmessagenum);
				$('.newletter').html('私信   '+data.newletter);
				$.each(data, function(i,val){ 
				    if(typeof(i)=="string"){
				    	$('#middle').find('.number'+i+'').addClass('numubercss');
				    	$('#middle').find('.number'+i+'').html(val);
				    }
				});  
			}
		},'json');
	...
	// 		if(data>0){
	// 	 		$('.newmessagenum').remove();
	// 			var newmessagenum = $('<i>'+data+'</i>');
	// 			newmessagenum.addClass('newmessagenum');
	// 	 		$('.icon-msg').append(newmessagenum);
	// 			$('.newletter').html('私信   '+data);
	// 		}
	// 	},'json');
...
//我自己的实时@me提醒
	...
	setInterval(function(){
		$.post(atmemeurl,{},function(data){
			if(data.newatme>0){
		 		$('.newmessagenum').remove();
				var newmessagenum = $('<i>'+parseInt(data.newatme+data.newletter)+'</i>');
				newmessagenum.addClass('newmessagenum');
		 		$('.icon-msg').append(newmessagenum);
				$('.newatme').html('@我   '+data.newatme);
			}
		},'json');
